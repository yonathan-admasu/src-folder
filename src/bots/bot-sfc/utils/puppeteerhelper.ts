...
const launchBrowser = async (headless: boolean): Promise<Browser> => {
  try {
    const launchOptions: PuppeteerLaunchOptions = {
      userDataDir: path.resolve(__dirname, "../temp/chrome"),
      headless,
      args: [
...
const getAttrMultiple = async (
  ...
  try {
    ...
    const attrs: string[] = await Promise.all(
      elementHandles.map(async (elementHandle) => {
        const content = await elementHandle.evaluate((elm, attribute) => {
          const value = elm.getAttribute(attribute);
          return value ? value.trim() : "";
        }, attribute);
        return content;
...
const getTxt = async (selector: string, page: Page) => {
  try {
    const elementHandle = await page.$(selector);
    if (elementHandle) {
      const txt = elementHandle.evaluate((el) =>
...
export { launchBrowser, launchPage, getAttr, getAttrMultiple, getTxt };
